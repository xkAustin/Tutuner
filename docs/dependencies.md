...
审查日期：2026-08-09。版本由 `pubspec.lock` 锁定；下表记录直接依赖的解析版本、用途、平台覆盖与许可证。

| 依赖 | 用途 | 目标平台 | 许可证 |
|---|---|---|---|
| `app_settings` 8.0.3 | 麦克风权限被永久拒绝后打开应用系统设置 | Android / iOS / macOS | MIT |
...
- `app_settings` 为权限错误卡片提供系统设置入口；当前版本支持 Flutter Swift Package Manager，不请求额外权限。Windows 继续使用界面中的系统路径说明。
- Flutter 3.44.8 的 Android 构建会提示 `app_settings` 尚未迁移到未来的 Built-in Kotlin；当前构建可用，后续升级 Flutter 前应复查插件兼容性。
...
- 82 个锁定 Pub 包和 Android Debug 运行时的 89 个 Maven 坐标已于审查日查询 OSV，修复后无命中。
